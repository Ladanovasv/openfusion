...
class Dataset(object):
    ...
    def __next__(self):
        if self.current < self.max_frames:
            print(len(self.rgbs_list), self.current)
    ...
    def load_color(self):
        rgbs_list = sorted(
            glob.glob(self.data_path + '/color/*.jpg'),
            key=lambda p: int(p.split("/")[-1].rstrip('.jpg'))
        )
        if len(rgbs_list) == 0:
            rgbs_list = sorted(
                glob.glob(self.data_path + '/color/*.png'),
...
class Replica(Dataset):
    ...
    def load_color(self):
        rgbs_list = sorted(
            glob.glob(self.data_path + '/color/*.jpg'),
            key=lambda p: p.split("/")[-1].rstrip('.jpg')
        )
        if len(rgbs_list) == 0:
            rgbs_list = sorted(
                glob.glob(self.data_path + '/color/*.png'),
                key=lambda p: p.split("/")[-1].rstrip('.png')
            )
        if len(rgbs_list) == 0:
            rgbs_list = sorted(
                glob.glob(self.data_path + '/rgb/*.png'),
...
class SberRobotics(Dataset):
    def scenes(self):
        return ["Sber_2", "Sber_3"]

    def load_intrinsics(self, img_size, input_size):
        intrinsic = np.zeros((3, 3))
        intrinsic[0, 0] = 910.4989013671875
        intrinsic[1, 1] = 910.498779296875
        intrinsic[0, 2] = 640.0
        intrinsic[1, 2] = 360.0
        return custom_intrinsic(intrinsic, *img_size, *input_size)
