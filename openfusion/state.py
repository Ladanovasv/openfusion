...
class VLState(BaseState):
    ...
    def fast_object_query(self, t_emb, points, colors=None, only_poi=False, topk=1):
        ...
        # print(f"Emb dict {self.emb_dict}")
        # print(f"Emb pcd {self.emb_coords}")
    ...
    def semantic_query(self, t_emb, points, colors=None, cmap=None):
        ...
        embed = self.emb_dict[valid_keys].to(t_emb.device)
        print(f"Emb dict {self.emb_dict.shape}")#
        print(f"Embed {embed.shape}")#
        print(f"Emb emb_coords {self.emb_coords.shape}")
        print(f"Emb key {self.emb_keys.shape}")
        print(f"Buffer indices {buf_indices.shape}")
        print(f"Mask key {mask_key.shape}")#
        print(f"Mask conf {mask_conf.shape}")
        print(f"Valid keys {valid_keys.shape}")
        ...
        labels = [np.argmax(np.bincount(m, weights=1/(1+d))) for d, m in zip(dist, semseg[ind])]
...
class CFState(BaseState):
    ...
    def semantic_query(self, t_emb, points, colors, cmap=None):
        ...
        embed = self.embed.to(t_emb.device)[buf_indices]
        mask_pred_caption = embed / (embed.norm(dim=-1, keepdim=True) + 1e-7)
        semseg = torch.einsum("cd,nd->cn", t_emb.cpu(), mask_pred_caption.cpu()).argmax(0).cpu().numpy() # (N,)
